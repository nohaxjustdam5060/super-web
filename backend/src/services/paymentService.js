...
class PaymentService {
   ...
   */
  async createPayment({ transaction_amount, token, description, installments, payment_method_id, payer, external_reference }) {
    try {
      logger.info(`[PaymentService] Processing payment for external_reference: ${external_reference}`);

      // Sandbox simulated approval for mock tokens if MP is in test mode or token is sandbox mock
      if (token === 'mock_token_approved' || process.env.MP_ACCESS_TOKEN.startsWith('TEST-')) {
        return {
          id: `MP-MOCK-${Date.now()}`,
          status: 'approved',
      ...
      return {
        ...
        card_last_four: response.card?.last_four_digits || '0000',
