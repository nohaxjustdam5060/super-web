...
class EmailService {
  ...
  async sendEmail({ to, subject, html, text }) {
    try {
      const fromAddress = process.env.EMAIL_FROM || 'SUPERLAPTOP Store <onboarding@resend.dev>';
  ...
  async sendOrderConfirmation(userEmail, order) {
    const subject = `Confirmación de Pedido #${order.order_number || ''} - SUPERLAPTOP Store`;
  ...
  async sendOrderStatusUpdate(userEmail, orderNumber, newStatus) {
    const subject = `Actualización de Pedido #${orderNumber} - SUPERLAPTOP`;
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e5e7eb; border-radius: 8px;">
        ...
        <p style="font-size: 12px; color: #6b7280;">Gracias por elegir SUPERLAPTOP Store.</p>
      </div>
    `;
  ...
  async sendPasswordReset(userEmail, resetLinkOrToken) {
    const subject = 'Restablecimiento de Contraseña - SUPERLAPTOP Store';
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e5e7eb; border-radius: 8px; padding: 20px;">
        ...
        <div style="padding: 20px 0;">
          ...
          <p style="color: #4B5563;">Has solicitado restablecer la contraseña de tu cuenta en SUPERLAPTOP.</p>
